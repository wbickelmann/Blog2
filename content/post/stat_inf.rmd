...
H0 There is no association between racial group and opinion on how much the government should do since 2000
H1 There is an association between racial group and opinion on how much the government should do  since 2000
* * *
...
According to the summaries generated above, the two variables the address the question (race and helpnot) are categorical variables with three levels each. This is helpful in determining what type of inference is necesarry. First it could be helpful to see the breakdown of opinion by race visually. A bar chart seems the most direct way to do this.
```{r}
...
View(gss.income)
cor(gss.income$incom16,gss.income$childs)
child.model<-lm(childs ~ incom16, data=gss.income)
summary(child.model)
```

Next step is to perform the Chi-Squared test on the data. In r this task is easily performed by the chisq.test function. Once a p value is derived from this method, the null hyopthesis can either be accepted or rejected.
```{r}
...
```
According to the p value from the chi-square test, the null hypothesis must be rejected. There is a relationship between racial group and opinion on how much the government should do.
